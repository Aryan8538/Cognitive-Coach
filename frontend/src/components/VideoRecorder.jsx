...
export default function VideoRecorder({ onRecordingComplete, isProcessing }) {
  ...
  // Define helper formats first to avoid hoisting warnings in useEffect refs
    ...
    <div className="glass-panel w-full md:max-w-md bg-white/70 dark:bg-zinc-900/55 backdrop-blur-lg border border-slate-200/50 dark:border-zinc-800/50 p-6 rounded-2xl shadow-sm hover:shadow-md hover:border-[#D4AF37]/20 dark:hover:border-[#D4AF37]/20 transition-all duration-300 animate-fade-in-up font-sans">
      ...
      <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-zinc-850/50 mb-4">
        ...
        {permission && (
          <div className="flex items-center gap-3">
            {recording ? (
              <div className="flex items-center gap-1.5 bg-rose-500/10 dark:bg-rose-500/15 px-2.5 py-1 rounded-full text-[10px] font-bold text-rose-650 dark:text-rose-450 border border-rose-500/25 animate-pulse-slow">
                <Circle size={6.5} fill="currentColor" className="text-rose-600 dark:text-rose-400 animate-pulse" />
                REC {formatTime(duration)}
              </div>
            ) : (
              <span className="text-[9px] uppercase tracking-widest font-extrabold text-slate-400 dark:text-slate-500">Standby</span>
            )}
        ...
        {!permission ? (
          ...
          <>
            ...
            {recording && currentTranscript && (
              <div className="absolute bottom-3.5 inset-x-3.5 bg-black/60 backdrop-blur-sm border border-white/10 text-white rounded-xl p-2.5 text-center text-[10.5px] font-medium leading-normal max-h-16 overflow-y-auto pointer-events-none z-20 font-mono">
            ...
            {!recording && (
              <div className="absolute bottom-3.5 inset-x-3.5 bg-slate-900/85 backdrop-blur-md border border-white/10 rounded-xl p-3 z-20 flex flex-col gap-1.5 items-center">
      ...
      {permission && !isProcessing && (
        <div className="flex justify-center gap-4 mt-5">
          {!recording ? (
            <button 
              className="px-6 py-2.5 bg-gradient-to-r from-rose-500 to-rose-600 hover:from-rose-600 hover:to-rose-700 text-white font-bold text-xs rounded-xl shadow-md transition-all duration-300 active:scale-95 flex items-center gap-1.5 cursor-pointer"
